...
const ACTION_ID = "kumbaya.post.content" as const;
const ROUTE_ID = "/api/agent/kumbaya";
const VENUE = "kumbaya";
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    ...
    nextAction:
      "POST with { action: 'kumbaya.post.content', content, walletProfileId?, target?, transaction? }.",
    policy: gate.policy,
  });
}
...
export async function POST(req: NextRequest) {
    ...
    return NextResponse.json(
      {
        ...
        error: {
          ...
          message: "content is required for kumbaya.post.content",
        },
      },
      { status: 400 }
    );
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: "POLICY_DENIED",
          message: "Policy denied kumbaya connector action.",
  ...
  const plan = {
    ...
    target: readString(body.target) || "kumbaya-feed",
  ...
  if (!executeRequested) {
    return NextResponse.json({
      ...
      requiresApproval: true,
      plan,
      proof: buildActionProof({ kind: "kumbaya.post.plan", plan }),
  ...
  return NextResponse.json({
    ...
    requiresApproval: true,
    execution: result,
    proof: buildActionProof({ kind: "kumbaya.post.execute", plan: result }),
