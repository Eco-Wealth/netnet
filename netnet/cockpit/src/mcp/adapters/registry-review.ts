...
export default class RegistryReviewAdapter implements MCPAdapter {
  chain: MCPChain = "registry-review";
  private endpoint: string;
  private apiKey?: string;

  constructor() {
    this.endpoint = String(process.env.REGISTRY_REVIEW_MCP_URL || "").trim();
    this.apiKey =
      String(process.env.REGISTRY_REVIEW_MCP_API_KEY || "").trim() || undefined;
  }

  private stub(req: MCPRequest): MCPResponse {
    if (req.method === "latest_block") {
      return { ok: true, data: { latestBlock: 0, stub: true } };
    }
  ...
  async request(req: MCPRequest): Promise<MCPResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        return { ok: false, error: `registry_review_mcp_http_${response.status}` };
      }
      ...
      if (req.method === "latest_block") {
        ...
        return { ok: false, error: "registry_review_mcp_invalid_latest_block" };
      }
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "registry_review_mcp_request_failed",
      };
    }
  }
}
