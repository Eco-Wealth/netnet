...
export default class RegenPythonAdapter implements MCPAdapter {
  chain: MCPChain = "regen-python";
  private endpoint: string;
  private apiKey?: string;

  constructor() {
    this.endpoint = String(process.env.REGEN_PYTHON_MCP_URL || "").trim();
    this.apiKey =
      String(process.env.REGEN_PYTHON_MCP_API_KEY || "").trim() || undefined;
  }
  ...
  async request(req: MCPRequest): Promise<MCPResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        return { ok: false, error: `regen_python_mcp_http_${response.status}` };
      }
      ...
      if (req.method === "latest_block") {
        ...
        return { ok: false, error: "regen_python_mcp_invalid_latest_block" };
      }
      ...
      return {
        ok: false,
        error:
          error instanceof Error ? error.message : "regen_python_mcp_request_failed",
      };
    }
  }
}
